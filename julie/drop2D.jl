...
# Surface tension simulation in 2D
...
##geometrical
const dr = 1.85e-5          #average particle distance (decrease to make finer simulation)
...
const beta = 52e-2      #surface tension
const vol = dr^2
const g = -9.8*VECY
...
function make_system()
    ...
    drop = Circle(0., rad+h, rad)
    #desk = Box(-2rad, -2rad, -deskw, 2rad, 2rad, 0.)
    desk = Rectangle(-2rad, -deskw, 2rad, 0.)
    #dom = Box(-2rad, -2rad, -2deskw, 2rad, 2rad, 2.2rad)
    dom = Rectangle(-2rad, -2deskw, 2rad, 2rad+h)
...
@inbounds function find_n!(p::Particle, q::Particle, r::Float64)
    p.n += 2*vol*vol*rDwendland2(h,r)*(p.x - q.x)
end
...
@inbounds function internal_force!(p::Particle, q::Particle, r::Float64)
        ...
        #surface tension
        p.a -= 2*beta/rho0^2*(
            (m*DDwendland2(h,r)-ker)*dot(p.x-q.x, p.n-q.n)*(p.x-q.x)/(r^2 + s0)
            +ker*(p.n-q.n)
        )
end
...
@fastmath function DDwendland2(h::Float64, r::Float64)::Float64
    x = r/h
    if (x > 1.0)
        return 0.0
    end
    # -30/pi ≈ -9.54929658551372
    return -9.54929658551372 * (1.0 - 2.0*x) * (1.0 - x)^2 / h^4
end
...
function main()
    E0 = 0.0
	sys = make_system()
	out = new_pvd_file("results/drop2D")
    save_frame!(out, sys, :v, :a, :P, :rho, :rho0, :type, :n)
    #initialization
    create_cell_list!(sys)
