...
const dr = W/20 #W/40
const h = (3.0+10.0*eps(Float64))dr
const m0 = rho0*dr*dr
...
#ALGEBRAIC TOOLS
#---------------
@fastmath function safeinv(x::Float64)::Float64
   return x/(x*x + EPS)
end
...
function check_L_stats!(sys)
    for (i,p) in enumerate(sys.particles)
        d = det(p.L)
        # count nonzero entries in L (rough sanity)
        nnz = (abs(p.L[1,1])>0) + (abs(p.L[1,2])>0) + (abs(p.L[2,1])>0) + (abs(p.L[2,2])>0)
        if !isfinite(d) || abs(d) < 1e-12
            println("particle $i: det(L) = $d, nnz in L = $nnz, J = $(p.J), K = $(p.K)")
        end
    end
end


function find_L!(p::Particle, q::Particle, r::Particle, rq::Float64, rr::Float64)
    ker_q = wendland2(h, rq)
    ker_r = wendland2(h, rr)
    ker = ker_q*ker_r
    x_rq = q.x-r.x
    X_rq = q.X-r.X
    p.A += ker*outer(X_rq, x_rq)
    p.L += ker*outer(x_rq, x_rq)
end    
...
function main()
    ...
    @time for k = 0 : Int64(round(t_end/dt))
        ...
        if (k % Int64(round(dt_plot/dt)) == 0)
            ...
            save_frame!(out, sys, :v, :A, :P, :L, :X)
        end
        #= if (k % Int64(round(dt_plot/dt)) == 0) 
            save_frame!(out, sys, :v, :A, :L, :P, :X)
        end  =#
        apply!(sys, update_v!)
        apply!(sys, update_x!)
        create_cell_list!(sys)
