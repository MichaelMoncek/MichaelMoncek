    ...
    for k = 1:length(particles) 
        if j != k 
            new_acceleration = new_acceleration .+
                               find_acceleration(particle, particles[k])
        end 
    end 
...
function nbody(n, n_steps; dt = 0.01, dim = 2,
    ...
    for i = 1:n_steps
        ...
        #= Threads.@threads for j = 1:n
            new_acceleration = ntuple(_-> 0, dim)
            for k = 1:n 
                if j != k
                    new_acceleration = new_acceleration .+ find_acceleration(particles[j], particles[k])
                end 
            end
            particles[j] = move_particle(particles[j], new_acceleration, dt)
        end
         =#
        if output_images
            if i%plot_steps == 0
